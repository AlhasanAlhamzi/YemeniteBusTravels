...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  if(!empty($_POST['adminuname'])){
    ...
    $wtype=$_POST['adminwtype'];
    $wbrunch=$_POST['adminwbrunch'];
    $h_number=$_POST['adminhwork'];
    $salary=$_POST['adminsalary'];
    ...
    if($pwd==$cpwd){
      ...
      if(!empty($_FILES['adminuimg']['name'])){
        ...
        if(in_array($ext, $arr)){
          ...
          if(mysqli_query($con,"insert into employees(uname,fname,lname,pwd,salary,type,gender,user_img,w_hours,w_type,w_brunch,note,dates) values('$uname','$fname','$lname','$pwd',$salary,'$type','$gender','$uname.$ext',$h_number,'$wtype',$wbrunch,'$note',now())")){
      ...
      alert('The File Which You Are Uploaded Is Not Of img Type.Please Try Again Later.');
          ...
          if(mysqli_query($con,"insert into employees(uname,fname,lname,pwd,salary,type,gender,w_hours,note,dates) values('$uname','$fname','$lname','$pwd',$salary,'$type','$gender',$h_number,'$note',now())")){
            echo"<script>
            alert('Add Employee $uname Successfully.');
            document.location='admins.php?showtype=employees';
            </script>";
...
<div id="addemployee" class="modal fade" role="dialog">
  ...
  <select class="form-control" name='admintype' id="sel1">
    ...
    <label for="file">Salary:</label><br/>
      ...
      <select class="form-control" name="adminwtype" id="sel1">
