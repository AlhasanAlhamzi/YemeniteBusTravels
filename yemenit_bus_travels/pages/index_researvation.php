...
<!-- Modal -->
  ...
  <form action='' method='POST' class='signupform' enctype="multipart/form-data">
    ...
    <input type="text" name='resfname' placeholder='First Name:' class="form-control" id="pwd"><br/>
  ...
  <label for="pwd">Address:</label><br/>
    ...
    <select name="retype" id="retypes" class="form-control" onchange="
    var d=document.getElementById('retypes').value;
    if(d!=''){
      
    document.getElementById('reunames').style.display='block';
    }else{
      document.getElementById('reunames').style.display='none';
    }" > 
      <option value=""></option>
    ...
    while($data=mysqli_fetch_assoc($query)){?>
      ...
      echo $data['cost']; ?>"/>
    <?php } ?>
      <lable id="restcost" name="retcost"></lable>
      </div>
