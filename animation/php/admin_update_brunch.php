...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  if(!empty($_POST['bruname'])){
    $id=mysqli_real_escape_string($con,$_POST['bruid']);
    $mid=mysqli_real_escape_string($con,$_POST['brumid']);
  ...
  <form action='' method='POST' class='signupform' enctype="multipart/form-data">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" onclick="document.location='admins.php?showtype=brunchs&actives=admin'; ">X</button>
        <h4 class="modal-title signtitle"><a class="fa fa-fw fa-user icons"/></a>ADD Brunch.</h4><br/>
        <?php 
        $query=mysqli_query($con,"select * from brunchs where id=".mysqli_real_escape_string($con,$_GET['br_id']));
  ...
  <?php }  ?>
    ...
    <label>Date: <?php echo $data['dates']; ?></label>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-danger" onclick="document.location='admins.php?showtype=brunchs&actives=admin';">Close</button>
      </div>
    </div>
