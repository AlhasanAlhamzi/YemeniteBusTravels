...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  if(!empty($_POST['truname'])){
    $empid=mysqli_real_escape_string($con,$_POST['truempid']);
    $id=mysqli_real_escape_string($con,$_POST['truid']);
    $brid=mysqli_real_escape_string($con,$_POST['trubrid']);
  ...
  <form action='' method='POST' class='signupform' enctype="multipart/form-data">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" onclick="document.location='admins.php?showtype=travels&actives=admin';">X</button>
        <h4 class="modal-title signtitle"><a class="fa fa-fw fa-user icons"/></a>ADD Travel.</h4><br/>
        <?php
        $query=mysqli_query($con,"select * from travels where id=".mysqli_real_escape_string($con,$_GET['tr_id']));
  ...
  <?php } ?>
    ...
    <label>Date: <?php echo $data['dates']; ?></label>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-danger" onclick="document.location='admins.php?showtype=travels&actives=admin';">Close</button>
      </div>
    </div>
