...
<?php
    ...
    if(!empty($_GET['delete'])){
        mysqli_query($con,"delete from admin_chats where id=".mysqli_real_escape_string($con,$_GET['delete']));
//        echo "<script>document.location='admin_chats.php?';</script>";
    }
    if(!empty($_GET['like'])){ 
            mysqli_query($con,"update admin_chats set likes=likes+1 where id=".mysqli_real_escape_string($con,$_GET['like']));
    }
?>
    <head>
        <meta charset="utf-8">
    ...
    <body>
    <p class="alert alert-success hichat"><big><b>
        Welcome Every Body.<br/>
                Here We Can Descussion All Questions.<br/>
                Good Luck.
            </br></big></p>
   <div class="well">
    <?php
    ...
    if($data1['uname']==$_SESSION['uname']){

    ?>
       <div class="alert alert-info alert-dismissable adminchatcloseleft">
        <?php  }else{?>
    <div class="alert alert-success alert-dismissable adminchatcloseright">
        <?php }?>
        <?php if(!empty($_SESSION['uname'])){
            if($_SESSION['type']=="manager" || $_SESSION['uname']==$data1['uname']){
            ?>
        <a href="admin_chats.php?delete=<?php echo $data['id']; ?>&actives=Managers chat" class="close" aria-label="close">X</a>
        <?php }} ?>
        <!--The left side-->
        <div class="media">
        ...
        <div class="media-body">
            ...
            ?><a href="admin_chats.php?like=<?php echo $data['id']; ?>&actives=Managers chat" class="fa fa-fw fa-thumbs-up liked"><small><span class="badge"><?php echo $data00['likes']; ?></span></small>
