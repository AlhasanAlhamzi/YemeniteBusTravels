...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  if(!empty($_POST['userupdateuname'])){
    $id=mysqli_real_escape_string($con,$_POST['userupdateempid']);
    ...
    $h_number=",w_hours=".mysqli_real_escape_string($con,$_POST['userupdatehwork']);
    $salary=",salary=".mysqli_real_escape_string($con,$_POST['userupdatesalary']);
    $table="employees";
    }
    else{
    $h_number=" ";
    ...
    $gender=mysqli_real_escape_string($con,$_POST['userupdategender']);
    if($pwd==$cpwd){
...
<div id="updatemployee" ><!--class="modal fade" role="sdialog">-->
  ...
  <form action='' method='POST' class='signupform' enctype="multipart/form-data">
    <!-- Modal content-->
          ...
          $query0=mysqli_query($con,"select id,dates from researvations where $id=".$_SESSION['user_id']);
          if($data0=mysqli_fetch_assoc($query0)){
            ?>
            <p class="alert alert-warning"><span class="fa fa-fw fa-warning trwarning"></span> Your card number is: <?php echo $data0['id']; ?>.\nThe Travel Going Time is <?php echo $data0['dates']; ?> And You Must Pay Befor 24 Hours From Travel Gone Time Or Your Seats Will Researves To Another Person And Folow The Supervisor Notes.\n Thanks For Travels With Us.
            </p>
            <?php
          }
